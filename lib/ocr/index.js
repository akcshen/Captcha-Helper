...
export async function recognize(image, settings) {
  ...
  const url = toCalculateUrl(cfg.apiUrl);
...
/**
 * 将配置 URL 规范到 /calculate
 * @param {string} url
 */
function toCalculateUrl(url) {
  const trimmed = String(url || '').trim().replace(/\/+$/, '');
  if (!trimmed) return trimmed;
  if (/\/classification$/i.test(trimmed)) {
    return trimmed.replace(/\/classification$/i, '/calculate');
  }
  if (/\/calculate$/i.test(trimmed)) return trimmed;
  return `${trimmed}/calculate`;
}
